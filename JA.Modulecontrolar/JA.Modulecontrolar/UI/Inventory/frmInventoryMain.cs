...
namespace JA.Modulecontrolar.UI.Inventory
{
    public partial class frmInventoryMain : Form
    {
        ...
        private void cmdrptStationary_Click(object sender, EventArgs e)
        {
            if (Utility.gblnAccessControl)
            {
                if (!Utility.gblnChildPrivileges(strComID, Utility.gstrUserName, 211))
                {
                    MessageBox.Show("You have no Permission to Access", "Privileges", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            if (System.Windows.Forms.Application.OpenForms["frmRptStationary"] as frmRptStationary == null)
            {
                frmRptStationary objfrm = new frmRptStationary();
                objfrm.MdiParent = this.MdiParent;
                objfrm.Show();
            ...
            {
                frmRptStationary objfrm = (frmRptStationary)Application.OpenForms["frmRptStationary"];
                objfrm.Focus();
                objfrm.MdiParent = this.MdiParent;
            }
        }
