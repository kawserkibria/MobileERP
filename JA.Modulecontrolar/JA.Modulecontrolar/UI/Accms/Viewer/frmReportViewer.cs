            ...
            try
            {
                 switch (selector)
                {
                    ...
                    case ViewerSelector.BankRP:
                        if (intSummDetails == 0)
                        {
                            string strInsert = objWoIS.mInserRPBank(strComID, strFdate, strTdate);
                            if (strInsert != "")
                            {
                                List<RAccountsGroup> oreport = objWoIS.mGetBankwiseQuery(strComID, 0, "", "");
                        ...
                        {
                            double dblAmount = 0;
                            List<RAccountsGroup> oreport = objWoIS.mGetBankwiseQuery(strComID, 1, strFdate, strTdate);
                            ...
                            rpt1.SetDataSource(oreport.ToList());
                            this.reportTitle = "Collection Deposit Summary";
                            this.secondParameter = Convert.ToDateTime(strFdate).ToString("dd-MM-yyyy") + " to " + Convert.ToDateTime(strTdate).ToString("dd-MM-yyyy");
                    ...
                    case ViewerSelector.MarketMonitoring:
                        List<RFinalStatement> oMarketMonitor = orptCnn.mGetMarketMonitoringSheet(strComID, strFdate, strTdate, strBranchID, intNarration, strString).ToList();
                    ...
                    case ViewerSelector.FinalStatement:
                        ...
                        List<RFinalStatement> oFinalState = orptCnn.mGetFinalStattemnet(strComID, strFdate, strTdate, strBranchID, strGroupName, strPartyName, 
                                                                                            intNarration, intSummDetails, dblClosingBalance, strSelction,Utility.gstrUserName).ToList();
                    ...
                    case ViewerSelector.DailyCollection:
                        List<RAccountsGroup> odailyColeDetails = objWoIS.mGetDailyCollectionDetails(strComID, strFdate, strTdate, strBranchID, intSP,Utility.gstrUserName).ToList();
                        if (odailyColeDetails.Count > 0)
                        {
                            if (intSP == 1)
                            {
                                ...
                                rpt.Subreports[0].SetDataSource(orptCnn.mGetDailyCollection(strComID, strFdate, strTdate, strBranchID, Utility.gstrUserName).ToList());
                                rpt.Subreports[1].SetDataSource(orptCnn.mGetDailyCollection(strComID, strTdate, strTdate, strBranchID, Utility.gstrUserName).ToList());
                    ...
                    case ViewerSelector.BalanceSheet:
                        ...
                        int oStock = orptCnn.mGetBalanceSheet(strComID, dteFdate, dteTdate, intHor_ver);
                        if (oStock == 1)
                        {
                    ...
                    case ViewerSelector.JVSP:
                        ...
                        rpt1.SetDataSource(orptCnn.mGetAccountsvoucherSP(strComID, intVtype, intSummDetails, strString, strBranchID).ToList());
                        this.reportTitle = strHeading;
                    ...
                    case ViewerSelector.Ledger:
                        ...
                        List<RAccountsGroup> oAccvou = objWoIS.RefreshLedger(strComID, strFdate, strTdate, strString, strBranchID, strSelction).ToList();
                        if (oAccvou.Count > 0)
                        {
                            if (intSP > 0)
                            {
                    ...
                    case ViewerSelector.TargetAchievement:
                        {
                            List<RAccountsGroup> otarget = orptCnn.GetrptCollectionTargetAchieve(strComID, strBranchID, strSelction, strString, strFdate, strTdate, intSummDetails, intNarration).ToList();
                            if (intSP == 1)
                            {
                            ...
                            this.reportTitle = "Sales Target & Collection Target Achievement ";
                            this.secondParameter = strFdate + " to " + strTdate;
                            InitialiseLabels(rpt1);
                    ...
                    case ViewerSelector.ExpenseSumm:
                        ...
                        InitialiseLabels(rpt1);
                        string strBackYear = "";
                        double dblBalanceBF = 0, dblBalanceBF1 = 0, dblCashReceived = 0, dblBanlReceived = 0;
                        if (intSummDetails == 0)
                        {
                            strBackYear = Convert.ToDateTime(strFdate).ToString("MMMM-yyyy");
                            //dblBalanceBF = Utility.gdblBalanceBF(strComID, strFdate, strTdate);
                            //dblBalanceBF = Utility.gdblBalanceBFLedger(strComID, "Cash Of Head Office", strFdate, strTdate);
                            dblBalanceBF = Utility.gGetdblLedgerClosingBalance(strComID, strFdate, strTdate, "Cash Of Head Office", "");
                            dblBalanceBF1 = Utility.gdblBalanceBFLedger(strComID, "Cash In Hand Factory", strFdate, strTdate);
                            //dblBalanceBF = Math.Abs(dblBalanceBF) + Math.Abs(dblBalanceBF1);
                        }
                        else
                        {
                            strBackYear = Convert.ToDateTime(strFdate).ToString("MMMM-yyyy");
                    ...
                    case ViewerSelector.Audit:
                        List<RAudit> oAudit = orptCnn.mGetAudit(strComID, strFdate, strTdate, intVtype, strString).ToList();
                        if (oAudit.Count > 0)
                        {
